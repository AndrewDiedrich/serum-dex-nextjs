...
import { exchangeMarketState, walletState } from '../recoil/exchangeState'
...
export function useMarketsList() {
  return USE_MARKETS.filter(
    ({ name, deprecated }) => !deprecated && !process.env.REACT_APP_EXCLUDE_MARKETS?.includes(name)
  )
}
...
export function useUnmigratedOpenOrdersAccounts() {
  ...
  async function getUnmigratedOpenOrdersAccounts(): Promise<OpenOrders[]> {
    ...
    const deprecatedProgramIds = Array.from(
      new Set(USE_MARKETS.filter(({ deprecated }) => deprecated).map(({ programId }) => programId.toBase58()))
    ...
    for (programId of deprecatedProgramIds) {
      try {
        ...
        deprecatedOpenOrdersAccounts = deprecatedOpenOrdersAccounts.concat(
          openOrdersAccounts
            .filter((openOrders) => openOrders.baseTokenTotal.toNumber() || openOrders.quoteTokenTotal.toNumber())
            .filter((openOrders) =>
              USE_MARKETS.some((market) => market.deprecated && market.address.equals(openOrders.market))
            )
        )
      } catch (e) {
...
const MarketContext: React.Context<null | MarketState> = React.createContext<null | MarketState>(null)
...
export const DEFAULT_MARKET = USE_MARKETS.find(({ name, deprecated }) => name === 'SRM/USDT' && !deprecated)
...
export function useMarket() {
  const context = useRecoilState(exchangeMarketState)
  if (!context) {
...
export function useMarkPrice() {
  ...
  useEffect(() => {
    ...
    const markPrice = bb && ba ? (last ? [bb, ba, last].sort((a, b) => a - b)[1] : (bb + ba) / 2) : null
...
export function _useUnfilteredTrades(limit = 10000) {
  // const { name } = useRecoilValue(exchangeMarketState)
  // const connection = useConnection()
  // async function getUnfilteredTrades(): Promise<any[] | null> {
  //   if (!name || !connection) {
  //     return null
  //   }
  //   return await name.loadFills(connection, limit)
  // }
  // const [trades] = useAsyncData(getUnfilteredTrades, tuple('getUnfilteredTrades', name, connection), {
...
export function useOrderbookAccounts() {
  const market = useRecoilValue(exchangeMarketState)
  // @ts-ignore
  const bidData = useAccountData(market && market._decoded.bids)
  // @ts-ignore
  const askData = useAccountData(market && market._decoded.asks)
  return {
    bidOrderbook: market.name && bidData ? Orderbook.decode(market, bidData) : null,
    askOrderbook: market.name && askData ? Orderbook.decode(market, askData) : null,
  }
}
...
export function useOrderbook(depth = 20): [{ bids: number[][]; asks: number[][] }, boolean] {
  ...
  const { name } = useRecoilValue(exchangeMarketState)
  const bids = !bidOrderbook || !name ? [] : bidOrderbook.getL2(depth).map(([price, size]) => [price, size])
  const asks = !askOrderbook || !name ? [] : askOrderbook.getL2(depth).map(([price, size]) => [price, size])
...
export function useOpenOrdersAccounts(fast = false) {
  const { name } = useRecoilValue(exchangeMarketState)
  const { connected, wallet } = useRecoilValue(walletState)
  const connection = useConnection()
  async function getOpenOrdersAccounts() {
    ...
    return await name.findOpenOrdersAccountsForOwner(connection, wallet.publicKey)
  }
  return useAsyncData<OpenOrders[] | null>(
    getOpenOrdersAccounts,
    tuple('getOpenOrdersAccounts', wallet, name, connected),
...
export function useOpenOrders() {
  const { market, marketName } = useRecoilValue(exchangeMarketState)
...
export function useFeeDiscountKeys(): [
  ...
  const getFeeDiscountKeys = async () => {
    if (!connected || !wallet) {
      return null
    }
    if (!market) {
...
export function useFills(limit = 100) {
  const { marketName } = useRecoilValue(exchangeMarketState)
...
export function useAllOpenOrdersAccounts() {
  const { wallet, connected } = useRecoilValue(walletState)
  const connection = useConnection()
  const marketInfos = useMarketInfos()
  const programIds = [...new Set(marketInfos.map((info) => info.programId.toBase58()))].map(
...
export function useBalances(): Balances[] {
  ...
  const { market, baseCurrency, quoteCurrency } = useRecoilValue(exchangeMarketState)
...
export function useWalletBalancesForAllMarkets(): {
  ...
  const { connected } = useRecoilValue(walletState)
  const [mintInfos, mintInfosConnected] = useMintInfos()
...
export function useBalancesForDeprecatedMarkets() {
  ...
  markets.forEach(({ market, openOrdersList }) => {
    const { baseCurrency, quoteCurrency, marketName } = getMarketDetails(market, customMarkets)
...
export function useMarketInfos() {
  const { customMarkets } = useRecoilValue(exchangeMarketState)
