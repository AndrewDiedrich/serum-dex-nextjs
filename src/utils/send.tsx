...
import { define, union, array, literal } from 'superstruct'
...
export async function placeOrder({
  ...
  if (isNaN(price)) {
    notify({ message: 'Invalid price', type: EToastType.ERROR })
    return
  }
  if (isNaN(size)) {
    notify({ message: 'Invalid size', type: EToastType.ERROR })
    return
  }
  if (!wallet || !wallet.publicKey) {
    notify({ message: 'Connect wallet', type: EToastType.ERROR })
    return
  }
  if (!market) {
    notify({ message: 'Invalid  market', type: EToastType.ERROR })
    return
  }
  ...
  if (size < market.minOrderSize) {
    notify({ message: 'Size too small', type: EToastType.ERROR })
    return
  }
  ...
  if (price < market.tickSize) {
    notify({ message: 'Price under tick size', type: EToastType.ERROR })
    return
  }
...
export async function sendSignedTransaction({
  ...
  if (sendNotification) {
    notify({ message: sentMessage, type: EToastType.SUCCESS, txid })
  }
  ...
  if (sendNotification) {
    notify({ message: successMessage, type: EToastType.SUCCESS, txid })
  }
